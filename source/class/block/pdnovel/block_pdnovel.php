...
class block_pdnovel {
	...
	function fields() {
		return array(
				...
				'novelid' => array('name' => lang('block_pdnovel', 'blockclass_pdnovel_field_novelid'), 'formtype' => 'text', 'datatype' => 'int'),
	...
	function getdata($style, $parameter) {
		...
		$pdmodule = DB::fetch_first('SELECT rewrite FROM '.DB::table('pdmodule_view'));
		while($data = DB::fetch($query)) {
			...
			if(empty($_G['cache']['pdnovelcategory'][$data['catid']]['upid'])) {
				$upid = $data['catid'];
			} else {
				$upid = $_G['cache']['pdnovelcategory'][$data['catid']]['upid'];
			}
			if($pdmodule['rewrite'] == 1 )
			{
				$url = 'novel-view-'.$data['novelid'].'.html';
				$upurl = 'novel-list-'.$upid.'.html';
				$caturl = 'novel-list-'.$data['catid'].'.html';
				$lastchapterurl = 'novel-read-'.$data['novelid'].'-'.$data['lastchapterid'].'.html';
			}
			else
			{
			...
			$list[] = array(
				...
				'fields' => array(
					...
					'author'=>$data['author'],
					'authorurl'=> 'pdnovel.php?mod=search&ac=author&srchtxt='.$data['author'],
