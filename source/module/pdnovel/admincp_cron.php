...
$cron['time'] .= 0 <= $cron['hour'] &&$cron['hour'] <24 ?sprintf( '%02d',$cron[hour] ).cplang( 'misc_cron_hour') : '';
...
$cron[lastrun],
$cron[nextrun],
"<a href=\"".ADMINSCRIPT.( '?action=pdnovel&operation=cron&edit='.$cron['cronid']."\" class=\"act\">{$lang['edit']}</a><br />").( $cron['available'] ?" <a href=\"".ADMINSCRIPT.( '?action=pdnovel&operation=cron&run='.$cron['cronid']."\" class=\"act\">{$lang['misc_cron_run']}</a>") : " <a href=\"###\" class=\"act\" disabled>".$lang['misc_cron_run'].'</a>')
) );
}
showtablerow( '',array(
'',
...
{
DB::update( 'common_cron',array( 'available'=>'0','nextrun'=>'0'),"cronid='".$cron['cronid']."'");
}
}
updatecache( 'setting');
...
else if ( !is_readable( DISCUZ_ROOT.( $cronfile = './source/include/cron/'.$_G['gp_filenamenew'] ) ) )
{
cpmsg( 'crons_filename_invalid','','error',array(
'cronfile'=>$cronfile
) );
}
else if ( $_G['gp_weekdaynew'] == -1 &&$daynew == -1 &&$_G['gp_hournew'] == -1 &&$minutenew === '')
{
...
else if ( !file_exists( DISCUZ_ROOT.( $cronfile = './'.$action.'/include/cron/'.$cron[filename] ) ) )
{
cpmsg( 'crons_run_invalid','','error',array(
'cronfile'=>$cronfile
) );
}
else
